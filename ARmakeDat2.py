...
class ARdat:
    ...
    def __init__(self, bsnm, N, rs, ths, gCn, gR):
        ...
        oo.bsnm  = bsnm
        oo.gCs  = 1
        sigCmps= len(rs)   #  # of AR2 components creating 1 observed signal
        oo.gCn  = gCn;     oo.gR  = gR
        oo.TR  = 1;        oo.N   = N
        
        oo.sgnls    = _N.empty((oo.gCs, oo.N))
        oo.nzs     = _N.empty((oo.gR + oo.gCn, oo.N))
        oo.rs      = _N.empty(sigCmps);        oo.ths      = _N.empty(sigCmps);

        alfa = _N.empty(sigCmps*2, dtype=_N.complex)
        for c in xrange(sigCmps):   #  build signal components
            oo.rs[c]    = rs[c];     oo.ths[c]    = ths[c]
            th1pi= _N.pi*ths[c]

            alfa[2*c]  = rs[c]*(_N.cos(th1pi) + 1j*_N.sin(th1pi))
            alfa[2*c+1]  = rs[c]*(_N.cos(th1pi) - 1j*_N.sin(th1pi))

        ARcoeff          = (-1*_Npp.polyfromroots(alfa)[::-1][1:]).real
        oo.sgnls[0], y = createDataAR(N, ARcoeff, 0.1, 0.1)
    ...
    def ass(self, sigAmp, nzAmps):
        oo = self
        oo.obsvd   = _N.zeros((oo.TR, oo.N))

        oo.obsvd[0]  += sigAmp * oo.sgnls[0]
        if (oo.nzs is not None) and (nzAmps is not None) and (oo.gR > 1):
            In = _N.diag(nzAmps)
            oo.obsvd[0] += _N.sum(_N.dot(In, oo.nzs), axis=0)
        elif (oo.gR == 1):
            oo.obsvd[0] += nzAmps* oo.nzs
