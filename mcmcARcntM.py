...
#        oo.smpx[:] = 0
        ...
        for it in xrange(oo.burn+oo.NMC):
            ...
            oo.us, oo.rnM, oo.model = cntmdlMCMCOnlyM(cntMCMCiters, oo.J, oo.zs, oo.us, oo.rnM, oo.model, oo.y, oo.mrns[it], oo.mus[it], oo.mdty[it], oo.smpx)

            CtDistLogNorm(_cd.__NBML__, cts, rnM, oo.LN)
                ...
                for m in xrange(oo.N+1):
                    ...
                    for j in xrange(oo.J):  #  for ratio of this state
                        for jo in xrange(oo.J):
                            trms[jo] = _N.exp(oo.LN[m, jo] - oo.LN[m, j]) * ((oo.m[jo] * (1 - p[m])**oo.rnM[jo])/(oo.m[j]* (1 - p[m])**oo.rnM[j]))
                        rats[j] = 1 / _N.sum(trms)
                    #print rats

                    #print "m:  %(m)d    %(r)s" % {"m" : m, "r" : rats}
                    ...
                    z[:]  = 0
                    if (oo.model == _cd.__BNML__) and (smRN <= oo.y[m]):
                        z[0] = 1
                    else:
