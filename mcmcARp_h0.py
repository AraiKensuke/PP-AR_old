...
class mcmcARp(mcmcARspk.mcmcARspk):
    ...
    def gibbsSamp(self):  ###########################  GIBBSSAMPH
        ...
        #  H shape    100 x 9
        Hbf = patsy.bs(_N.linspace(0, 1.2, 1200), knots=_N.array([0, oo.h0_1*oo.dt, oo.h0_2*oo.dt, oo.h0_2*3*oo.dt, oo.h0_2*4*oo.dt, oo.h0_2*5*oo.dt, 1.2]), include_intercept=True)    #  spline basisp
        ...
        if False:#oo.h0_1 > 0:   #  first few are 0s
        ...
        while (it < runTO):
            ...
            for m in xrange(ooTR):
                lw.rpg_devroye(oo.rn, oo.us[m] + BaS, out=oo.ws[m])  ######  devryoe

            if ooTR == 1:
                oo.ws   = oo.ws.reshape(1, ooN+1)
            _N.divide(oo.kp, oo.ws, out=kpOws)    ##  kpOws
