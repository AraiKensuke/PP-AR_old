...
class mcmcARp:
    ...
    def initGibbs(self):   ################################ INITGIBBS
        ...
        oo.smp_u        = _N.zeros(oo.burn + oo.NMC)
        ...
        if (oo.rs < 0):
            ...
            for m in xrange(oo._d.TR):
                lw.rpg_devroye(oo.rn, oo.u, num=(oo.N + 1), out=oo.ws[m, :])
    ...
    def gibbsSamp(self):  ###########################  GIBBSSAMPH
        ...
        while (it < ooNMC + oo.burn - 1):
            ...
            for m in xrange(ooTR):
                lw.rpg_devroye(oo.rn, oo.u, out=oo.ws[m])  ###devryoe

            if ooTR == 1:
                oo.ws   = oo.ws.reshape(1, ooN+1)

            lv_u  = 1/_N.sum(oo.ws)        #  variance of likelihood normal
            lm_u = _N.sum(oo.kp) * lv_u #  mean of likelihood normal
            mean = (oo.u_u*lv_u + lm_u*oo.s2_u) / (lv_u + oo.s2_u)
            vari = (lv_u * oo.s2_u) / (lv_u + oo.s2_u)
            oo.u = mean + _N.sqrt(vari)*_N.random.randn()
